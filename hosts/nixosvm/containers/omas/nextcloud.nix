...
in
{
  ...
  services.nextcloud = {
    ...
    extraApps = {
      ...
      #NOTE We can put the files here to test the app without using nixos-rebuild:
      #     /var/lib/nixos-containers/omas/var/lib/nextcloud/store-apps/calendarcustomcss
      #calendarcustomcss = ./nextcloud_calendarcustomcss;
      calendarcustomcss = (pkgs.fetchNextcloudApp {
        url = "calendarcustomcss";
        license = "agpl3Plus";
      }).overrideAttrs (_: {
        src = ./nextcloud_calendarcustomcss;
      });
    };
